...
void modbus_SendCommand( tCommand c) {
	...
	// Comando para ler as amostras tiradas durante experimento de todos os termômetros.
...
void * modbus_Process(void * params) {
	...
	while (control.exit == 0){
		...
		// comando para ajuste dos reles, vamos sinalizar para não enviar mais comandos
			...
			control.getStatus = 0; // sinalizo para não pegar mais status
			// Comando para pegar quantas amostras foram tiradas durante experimento
			// Comando para o status do experimento
		}  else if (cmd == cmdGET_SAMPLES) {
			control.nSamples = regs[0];
			control.getSamples = 0; // sinalizo para não pegar mais status

		// Comando para ler os termômetros
		} else if (cmd == cmdGET_TEMP) {
			...
			control.getTemp = 0; // sinalizo para não pegar mais a temperatura

			// Comando para ler as amostras tiradas durante experimento de todos os termômetros.
		} else if (cmd == cmdGET_RESULTS) {
			for (x=0; x<control.nThermAccessSamples; x++) 
				control.thermBufferSamples[x] = regs[x];
			control.getResult = 0; // sinalizo para não pegar mais os resultados do experimento
		}
    }
...
NAN_METHOD(GetAllValues) {
	...
	control.thermAccessAddr= 0x1000; // Log do primeiro termômetro 
	control.getResult = 1;
